...
UserPromptSubmit Hook → cn2en.sh
    ...
    │  additionalContext 中的指示要求 Claude 优先使用英文译文
    ▼
...
终端显示英文输出
    ...
    ▼
Stop Hook → en2cn.sh
    │  读取 last_assistant_message → 翻译成中文
    │  通过 systemMessage 将中文翻译展示给用户
    ▼
终端显示中文翻译（系统消息）
```

> **技术说明：** Claude Code 的 UserPromptSubmit hook **不支持直接替换 prompt 文本**，只能通过 `additionalContext` 注入附加上下文。因此英文翻译会与中文原文一同传递给 Claude，但 hook 注入的指示会要求 Claude 以英文译文为准。Stop hook 的 stdout 仅写入 debug log，因此翻译结果通过 `systemMessage` 字段展示给用户。
...
安装完成后，**所有** Claude Code 项目都会自动启用翻译代理。重复运行安装脚本不会重复添加 hook。

> **注意：** Hook 配置写在 `~/.claude/settings.json` 而非 Plugin 目录，这是为了规避 Claude Code 已知 bug（[#10225](https://github.com/anthropics/claude-code/issues/10225)），该 bug 会导致 Plugin 中的 UserPromptSubmit hook 无法触发。仅运行安装脚本不会自动加载 `skills/chinese-router/SKILL.md`；当前实现依靠 `additionalContext` 中的指示生效。如果你通过 `claude --plugin-dir /path/to/claude-chinese-router-plugin` 加载插件，skill 也会作为补充指令可用。
...
编辑 `~/.claude/settings.json`，删除 `hooks.UserPromptSubmit` 和 `hooks.Stop` 中 `command` 指向本仓库 `scripts/cn2en.sh` / `scripts/en2cn.sh` 的条目。

不要直接删除整个 `hooks.UserPromptSubmit` 或 `hooks.Stop` 数组，里面可能有你自己的其他 hooks。可以用卸载脚本自动只删除 Chinese Router 条目。

如果 hooks 对象仅剩 Chinese Router 的配置，卸载脚本会自动清理空对象。
...
Claude Code 的 UserPromptSubmit hook **不支持直接替换 prompt 文本**——这是 Claude Code 本身的限制。本插件通过 `additionalContext` 注入英文翻译，并在同一个上下文中指示 Claude 以英文翻译为准进行响应。中文原文仍在上下文中，因此本插件不能保证“零中文接触”，只能实现“英文译文优先”。
